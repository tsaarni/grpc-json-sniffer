...
export class GrpcViewer {
  constructor() {
    ...
    // Templates
    this.messageListTemplate = document.getElementById(
      'message-list-template'
    ).content.firstElementChild;
    this.messageDetailsTemplate = document.getElementById(
      'message-details-template'
    ).content.firstElementChild;

    // Elements
    this.filterInput = document.getElementById('message-list-filter-input');
    this.filterInputClearButton = document.getElementById(
      'filter-clear-button'
    );
    ...
    this.filterHelpCloseButton = document.getElementById(
      'filter-help-close-button'
    );
  ...
  renderMessageList() {
    ...
    for (const msg of filteredMessages) {
      ...
      item.querySelector('.message-row-message-id').textContent =
        msg.message_id;
      item.querySelector('.message-row-timestamp').textContent =
        formatTimestamp(msg.time, this.timeZone);
      item.querySelector(
        '.message-row-method-and-message'
      ).textContent = `${stripNamespace(msg.method)} (${stripNamespace(
        msg.message
      )})`;
      ...
      item.addEventListener('click', () => {
        ...
        for (const el of this.messagesListContainer.querySelectorAll(
          '.message-row-content'
        )) {
  ...
  renderMessageDetails(msg) {
    ...
    details.querySelector('#message-details-message-id-value').textContent =
      msg.message_id;
    details.querySelector('#message-details-timestamp-value').textContent =
      formatTimestamp(msg.time, this.timeZone);
    details
      .querySelector('#message-details-method-value')
      .appendChild(this.createFilterLink('method', msg.method));
    details
      .querySelector('#message-details-message-value')
      .appendChild(this.createFilterLink('message', msg.message));
    details
      .querySelector('#message-details-direction-value')
      .appendChild(this.createFilterLink('direction', msg.direction));
    details
      .querySelector('#message-details-peer-address-value')
      .appendChild(this.createFilterLink('peer_address', msg.peer_address));
    details.querySelector('#message-details-payload-value').textContent =
      JSON.stringify(msg.content, null, 2);

    // Optional fields.
    if ('stream_id' in msg) {
      details
        .querySelector('#message-details-stream-id')
        .classList.remove('hidden');
      details
        .querySelector('#message-details-stream-id-value')
        .appendChild(this.createFilterLink('stream_id', msg.stream_id));
    }
    if ('error' in msg) {
      details
        .querySelector('#message-details-error')
        .classList.remove('hidden');
  ...
  handleArrowKey(event) {
    ...
    let selectedIndex = filteredMessages.findIndex(
      (msg) => msg.message_id === this.selectedMessageId
    );
